...
export default async function BillingPaymentsPage() {
  ...
  return (
    <AppShell user={user} title="Payments" subtitle="Review posted collections and remaining balances across billing claims.">
      <SectionCard eyebrow="Collections" title="Payment posture by claim">
        <DataTable columns={["Claim", "Patient", "Status", "Paid", "Balance"]}>
          {claims.map((claim) => (
            <tr key={claim.id}>
